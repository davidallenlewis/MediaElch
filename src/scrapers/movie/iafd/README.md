...
| Synopsis | `<outline>` | Overwrites with IAFD `Synopsis` if `Synopsis` exists | Existing NFO value kept |
| Scenes | `<plot>` | Overwrites with IAFD `Scene Breakdown` if it exists | Existing NFO value kept |
